...
```{r}
# Creating a subset of the arcos_county_pills_per_year data for Mingo County, West Virginia
...
```{r}
# A full and final graph for Mingo County that shows opioid shipments reached an astronomical peak in 2008 before declining each year until 2012. 
ggplot(mingo_pills_per_year) +
  geom_bar(stat="identity", aes(year, dosage_unit), fill="green") +
  labs(x="Year", y="Total pills", title="In Mingo County, opioids shipments peak in 2008 before steadily declining until 2012", subtitle = "Total pills shipped to Mingo County by year", caption = "Source: DEA ARCOS database, via Washington Post") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
  scale_y_continuous(labels = comma)

# Saved for the web.
ggsave("mingo_pills_per_year.png", plot=last_plot())
...
```{r}
# Creating a subset of the West Virginia opioid data that only contains entries for counties in 2012.
...
```{r}
# Pull in the county population data for West Virginia in 2012
...
```{r}
# Join population and opioid data for West Virginia in 2012
...
```{r}
# Create a scatter plot for West Virginian counties and opioid shipments in 2012
...
```
```{r}
# Only those results above 5 million pills labelled
ggplot(west_virginia_2012) +
  geom_point(aes(population, dosage_unit)) +
  labs(x="2012 Population", y="Total pills in 2012", title="Kanawha county, Raleigh county had high numbers of opioid pills relative to population in 2012", caption = "Source: DEA ARCOS database, via Washington Post", fill="buyer_county") +
  scale_y_continuous(labels = comma) +
  scale_x_continuous(labels = comma) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  geom_smooth(aes(population, dosage_unit), method = "lm", se = FALSE) +
  geom_text_repel(aes(population, dosage_unit, label=buyer_county),
  subset(west_virginia_2012, population >= 60000))

#Save a Web Version
ggsave("West_Virginia_County_Pills_2012.png", plot=last_plot())
