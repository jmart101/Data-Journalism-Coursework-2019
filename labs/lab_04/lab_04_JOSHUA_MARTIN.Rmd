...
```{r}
mingo_working <- mingo %>% mutate(reporter_address_complete = paste0(reporter_address1, " ", reporter_city, " , ", reporter_state, " ", reporter_zip))
...
```{r}
mingo_working <- mingo %>% 
  mutate(state_distribution = case_when(reporter_state == "WV" ~ "in_state_distribution",
...
```{r}
mingo %>%
  mutate(pill_pipeline = paste0(combined_labeler_name," ", reporter_name," ",  buyer_name)) %>%
  group_by(pill_pipeline) %>%
 summarise(total_pills = sum(dosage_unit)) %>%
  arrange(desc(total_pills))
#Actavis - Miami-Luken - Tug Valley Pharmacy, LLC with a total of 4954200 pills.
...
# I would need to use the case_when function to select and mutate them to a single value i.e by creating an equivalence between their current values and a single "CARDINAL HEALTH" value. 
