...
```{r}
# Load in four different datasets showing buyer addresses, buyer totals, buyers annual totals and staye populatons per year
...
# In the preface it mentioned some of the buyers had more and fewer annual buyer records per buyer. This was not the case with the totals. 

#My best guess for why there are more buyer addresses than buyer totals is that there are some buyers with multiple addresses, and the buyer totals tally up the total for all of those addresses under a single buyer. 
...
```{r}
# Use a simple arrange descending function to filter through 
buyer_totals %>% arrange(desc(total_pills))
# The South Carolina VA Consolidated Mail Outpatient Pharmacy (North Charleston) with 512414034 pills.
...
```{r}
# Apply three filters to the buyer_annual_by_year data to identify the practitioner who bought the most opioids in Maryland in 2010. Then, use a left join to identify the name of the doctor. 
...
# The doctor's name is MD Rakesh K Mathur.He had 337,450 pills as compared to Sriram Balasubramanian, with the next highest count of 212,800. According to the Maryland State Physician's records, Mathur is permanently banned from dispensing medication except in cases of emergency from July 27 2012 and is currently on probation. He was found guilty of "selling, prescribing, giving away, or administering drugs for illegal or illegitimate medical purposes".

# I would use the address 1 to check if they were the same as the records.
...
# The state with the highest rate of total pills per person over the 2006 to 2012, with 465 pills per person (rounded up).
```
...
# There 148, 831 rows with an inner join
buyer_totals %>%
left_join(buyer_addresses,by="buyer_dea_no")
# There are 2,043,330 rows with a left join
buyer_totals %>%
filter(is.na("buyer_address1"))
# There are 0 rows with a left join that only shows when they have a missing value. This means there no values missing in buyer addresses.
buyer_totals %>%
anti_join(buyer_addresses)
#  There are 0 rows with an anti_join that only shows when they do not have a matching value in the buyer_addresses table.
...
# There are 148,831 rows
buyer_addresses %>% left_join(buyer_totals,by="buyer_dea_no")
# There are 278,715 rows
buyer_addresses %>% filter(is.na(buyer_dea_no))
# There are 0 rows
buyer_addresses %>% anti_join(buyer_totals)
# There are 129, 925 rows returned which means there are values that are not in the buyer_totals table.
