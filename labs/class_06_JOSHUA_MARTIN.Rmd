...
```{r}
harford_raw <- county_raw(county="Harford", state="MD", key = key)

```
Caution: do NOT use the raw_data() function, it will almost certainly crash your computer. 

```{r}
prince_george_pharmacies <-total_pharmacies_county(county="Prince Georges", state="MD", key = key) 


```

```{r}
maryland_monthly_pill_count <- summarized_county_monthly(state="MD", key = key)
```
## Analysis
...
```{r}
college_park_pill_count <- total_pharmacies_county(county="Prince Georges", state="MD", key = key) 
# Opioid sales in university pharmacies, pharmacies in college towns. What is the rate of college opioid use across the country?

```

```{r}

# National comparison of opioid manufacturing in blue vs red states. Join with census data, and additional reporting to establish state laws on opioid manufacturing during 2006-2012 period.
california_pill_count <- total_manufacturers_state(state="CA", key = key)
texas_pill_count <- total_manufacturers_state(state="TX", key = key)
```

```{r}
# Opioid sales in states with legal recreational marijuana, and lenient drug laws compared to the rest of the country. 
District_Columbia_pill_count <- total_pharmacies_state(state="DC", key = key)

```

```{r}
# Opioid sales in former company towns, compared to the rest of the country. Create a subset of the data with only company towns. 


```

```{r}
# Compare opioid use, deaths to excessive alcohol use, deaths in the top highest consumers for each. Use CDC data for deaths and alcohol usage.
