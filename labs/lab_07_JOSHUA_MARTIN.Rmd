...
# Load Tidyverse, janitor and arcos, tidycensus, mapview, ggthemes, scales # install.packages(tidyverse)
...
```{r}
#Many of the records that do not show up within the population table are due to the fact they represent the American territories of Puerto Rico, Palau and the Virgin Islands. The other results omitted do not have a county or county fips code associated with the record. These also correspond to only the first few years of records, indicating poor record keeping?  AE actually represents a military base, an outlier and unique example. This could prejudice results later on by confusing or omitting areas.
```
...
geo_pills_population_state_2009 %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills Per Person',title="West Virginia received the most opioid pills per person in 2009", subtitle = "2009 population, U.S. Census and Opioid Pills Per Person, ARCOS data", caption = "Source: U.S. Census ACS and Washington Post ARCOS database") +
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="The rate of opioid pills per person increased nationally from 2006 to 2012, particularly in the East Coast states", subtitle = "", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
# Beyond the ballooning pills per person rates within the Appalachian region, Charleston county is the severest outlier both in South Carolina and out of state with nearly 314 pills per person. A few Mississipi counties are a hotspot, as well as Columbus County, North Carolina and Nye County, Nevada.  
```
...
geo_pills_population_county <- county_geodata_shifted %>%
 inner_join(pills_population_maryland, by=c("GEOID" = "countyfips"))

geo_pills_population_county %>%
  ggplot(aes(fill = pills_per_person)) +
  facet_wrap(~year, nrow=2) +
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Opioid pills per person in Maryland counties from 2006 to 2012", subtitle = "", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)

# 2011 was the worst year for the opioid pill per person rate, almost entirely statewide. The rate steadily increases from 2006-2011 before plateauing between 2010-2011 and then reducing slightly in 2012. Cecil County had the highest rate and steadily increased the entire period, before dramtically decreasing in 2012. 
